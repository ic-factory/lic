...
RSpec.describe "lic install" do
  context "git sources" do
    it "displays the revision hash of the gem repository", :lic => "< 2" do
      ...
      expect(the_lic).to include_gems "foo 1.0", :source => "git@#{lib_path("foo")}"
    end

    it "displays the ref of the gem repository when using branch~num as a ref", :lic => "< 2" do
      build_git "foo", "1.0", :path => lib_path("foo")
      ...
      expect(the_lic).to include_gems "foo 2.0", :source => "git@#{lib_path("foo")}"
    end
