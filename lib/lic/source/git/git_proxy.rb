    ...
    class Git
      ...
      class GitNotAllowedError < GitError
        def initialize(command)
          msg = String.new
          msg << "Lic is trying to run a `git #{command}` at runtime. You probably need to run `lic install`. However, "
          msg << "this error message could probably be more useful. Please submit a ticket at http://github.com/lic/lic/issues "
      ...
      class GitProxy
        ...
        def initialize(path, uri, ref, revision = nil, git = nil)
          ...
          @revision = revision
          @git      = git
          raise GitNotInstalledError.new if allow? && !Lic.git_present?
        end
        ...
        def checkout
          ...
          Lic.ui.info "Fetching #{URICredentialsFilter.credential_filtered_uri(uri)}"
        ...
        def copy_to(destination, submodules = false)
          ...
          unless File.exist?(destination.join(".git"))
            begin
              ...
              raise GitError, "Lic could not install a gem because it needs to " \
      ...
      private
        ...
        def git_null(command)
          git("#{command} 2>#{Lic::NULL}", false)
        end

        def git_retry(command)
          Lic::Retry.new("`git #{URICredentialsFilter.credential_filtered_string(command, uri)}`", GitNotAllowedError).attempts do
            git(command)
          end
        end
        ...
        def configured_uri_for(uri)
          if /https?:/ =~ uri
            remote = URI(uri)
            config_auth = Lic.settings[remote.to_s] || Lic.settings[remote.host]
        ...
        def allowed_in_path
          ...
          raise GitError, "The git source #{uri} is not yet checked out. Please run `lic install` before trying to start your application"
        end

        # TODO: Replace this with Open3 when upgrading to lic 2
