...
module Lic
  class Injector
    ...
    def self.inject(new_deps, options = {})
      ...
      injector.inject(Lic.default_gemfile, Lic.default_lockfile)
    end

    def self.remove(gems, options = {})
      injector = new(gems, options)
      injector.remove(Lic.default_gemfile, Lic.default_lockfile)
    end
    ...
    def inject(gemfile_path, lockfile_path)
      if Lic.frozen_lic?
        ...
        Lic.definition.ensure_equivalent_gemfile_and_lockfile(true)
      end

      # temporarily unfreeze
      Lic.settings.temporary(:deployment => false, :frozen => false) do
        ...
        @definition.lock(Lic.default_lockfile)

        # invalidate the cached Lic.definition
        Lic.reset_paths!
    ...
    def remove(gemfile_path, lockfile_path)
      ...
      Lic.definition.gemfiles.each do |path|
        deps = remove_deps(path)

        show_warning("No gems were removed from the gemfile.") if deps.empty?

        deps.each {|dep| Lic.ui.confirm "#{SharedHelpers.pretty_dependency(dep, false)} was removed." }
      end
    end
  ...
  private
    ...
    def remove_deps(gemfile_path)
      ...
      Lic.ui.info "Removing gems from #{gemfile_path}"
    ...
    def cross_check_for_errors(gemfile_path, original_deps, removed_deps, initial_gemfile)
      ...
      unless extra_removed_gems.empty?
        ...
        raise InvalidOption, "Gems could not be removed. #{extra_removed_gems.join(", ")} would also have been removed. Lic cannot continue."
      end
    ...
    def show_warning(message)
      Lic.ui.info Lic.ui.add_color(message, :yellow)
    end
  end
end
