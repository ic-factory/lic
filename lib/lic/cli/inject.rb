  ...
  class CLI::Inject
    ...
    def run
      ...
      gems.each_slice(4) do |gem_name, gem_version, gem_group, gem_source|
        ...
        deps << Lic::Dependency.new(gem_name, gem_version, "group" => gem_group, "source" => gem_source)
      end

      added = Injector.inject(deps, options)

      if added.any?
        Lic.ui.confirm "Added to Gemfile:"
        Lic.ui.confirm(added.map do |d|
          name = "'#{d.name}'"
        ...
        Lic.ui.confirm "All gems were already present in the Gemfile"
      end
    end
