...
module Lic
  class RubygemsIntegration
    ...
    def validate(spec)
      Lic.ui.silence { spec.validate(false) }
    ...
    def platforms
      return [Gem::Platform::RUBY] if Lic.settings[:force_ruby_platform]
      Gem.platforms
    end
    ...
    def configuration
      ...
      Lic.ui.error "#{e.class}: #{e.message}"
      Lic.ui.trace e
      raise
    ...
    def loaded_gem_paths
      ...
      if Gem::Specification.method_defined? :full_require_paths
        ...
        $LOAD_PATH.select do |p|
          Lic.rubygems.gem_path.any? {|gp| p =~ /^#{Regexp.escape(gp)}/ }
        end
      end
    end
    ...
    def fetch_all_remote_specs(remote)
      old_sources = Lic.rubygems.sources
      Lic.rubygems.sources = [remote.uri.to_s]
      ...
      Lic.rubygems.sources = old_sources
    end
    ...
    def download_gem(spec, uri, path)
      uri = Lic.settings.mirror_for(uri)
      fetcher = Gem::RemoteFetcher.new(configuration[:http_proxy])
      Lic::Retry.new("download gem from #{uri}").attempts do
    ...
    def replace_gem(specs, specs_by_name)
      ...
      [kernel, ::Kernel].each do |kernel_class|
        redefine_method(kernel_class, :gem) do |dep, *reqs|
          executables ||= specs.map(&:executables).flatten if ::Lic.rubygems.binstubs_call_gem?
          ...
          message = if spec.nil?
            "#{dep.name} is not part of the lic." \
            " Add it to your #{Lic.default_gemfile.basename}."
          else
        ...
        # backwards compatibility shim, see https://github.com/lic/lic/issues/5102
        kernel_class.send(:public, :gem) if Lic.feature_flag.setup_makes_kernel_gem_public?
      end
    end
    ...
    # Used to make bin stubs that are not created by lic work
    # under lic. The new Gem.bin_path only considers gems in
    # +specs+
    def replace_bin_path(specs, specs_by_name)
      ...
      redefine_method(gem_class, :find_spec_for_exe) do |gem_name, *args|
        ...
        unless spec
          ...
          if !exec_name || spec_with_name.nil?
            message += ". #{gem_name} is not currently included in the lic, " \
                       "perhaps you meant to add it to your #{Lic.default_gemfile.basename}?"
          end
        ...
        unless spec.name == gem_name
          Lic::SharedHelpers.major_deprecation 2,
            "Lic is using a binstub that was created for a different gem (#{spec.name}).\n" \
            "You should run `lic binstub #{gem_name}` " \
            "to work around a system/lic conflict."
        end
        spec
      end
      ...
      redefine_method(gem_class, :activate_bin_path) do |name, *args|
        exec_name = args.first
        return ENV["LIC_BIN_PATH"] if exec_name == "lic"
    ...
    # Because Lic has a static view of what specs are available,
    ...
    # Replace or hook into RubyGems to provide a licized view
    # of the world.
    ...
    class Future < RubygemsIntegration
      ...
      def gem_remote_fetcher
        ...
        dns = Resolv::DNS.new
        Lic::GemRemoteFetcher.new(proxy, dns)
      end
    ...
    class MoreFuture < Future
      ...
      def all_specs
        require "lic/remote_specification"
      ...
      def use_gemdeps(gemfile)
        ENV["LIC_GEMFILE"] ||= File.expand_path(gemfile)
        require "lic/gemdeps"
        runtime = Lic.setup
        Lic.ui = nil
