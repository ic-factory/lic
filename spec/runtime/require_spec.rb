...
RSpec.describe "Lic.require" do
  before :each do
    ...
    gemfile <<-G
      path "#{lib_path}" do
        ...
        env "LIC_TEST" => "nine" do
          gem "nine", :require => true
        end
        gem "ten", :install_if => lambda { ENV["LIC_TEST"] == "ten" }
      end
    G
  end
  ...
  describe "using lic exec" do
    it "requires the locked gems" do
      lic "exec ruby -e 'Lic.require'", :env => { :RUBYOPT => "-r#{spec_dir.join("support/hax")}" }
      expect(out).to eq("two")

      lic "exec ruby -e 'Lic.require(:bar)'", :env => { :RUBYOPT => "-r#{spec_dir.join("support/hax")}" }
      expect(out).to eq("baz\nqux")

      lic "exec ruby -e 'Lic.require(:default, :bar)'", :env => { :RUBYOPT => "-r#{spec_dir.join("support/hax")}" }
      expect(out).to eq("baz\nqux\ntwo")
    end
  end
  ...
  describe "order" do
    ...
    describe "a gem with different requires for different envs" do
      before(:each) do
        build_gem "multi_gem", :to_lic => true do |s|
      ...
      it "requires both with Lic.require(both)" do
        run "Lic.require(:one, :two)"
        expect(out).to eq("ONE\nTWO")
      end

      it "requires one with Lic.require(:one)" do
        run "Lic.require(:one)"
        expect(out).to eq("ONE")
      end

      it "requires :two with Lic.require(:two)" do
        run "Lic.require(:two)"
        expect(out).to eq("TWO")
      end
    end
    ...
    describe "with busted gems" do
      it "should be busted" do
        build_gem "busted_require", :to_lic => true do |s|
...
RSpec.describe "Lic.require with platform specific dependencies" do
