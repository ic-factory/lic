...
module Lic
  ...
  # the plugins to use.
  #
  # For plugins to be independent of the Lic internals they shall limit their
  # interactions to methods of this class only. This will save them from breaking
  # when some internal change.
  #
  # Currently we are delegating the methods defined in Lic class to
  # itself. So, this class acts as a buffer.
  #
  # If there is some change in the Lic class that is incompatible to its
  ...
  module Plugin
    class API
      autoload :Source, "lic/plugin/api/source"
      ...
      def self.source(source, cls = self)
        cls.send :include, Lic::Plugin::API::Source
      ...
      def tmp(*names)
        Lic.tmp(["plugin", *names].join("-"))
      end

      def method_missing(name, *args, &blk)
        return Lic.send(name, *args, &blk) if Lic.respond_to?(name)
      ...
      def respond_to_missing?(name, include_private = false)
        SharedHelpers.respond_to?(name, include_private) ||
          Lic.respond_to?(name, include_private) || super
      end
    end
  end
end
