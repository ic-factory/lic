...
module Gem
  ...
  class Specification
    ...
    def full_gem_path
      # this cannot check source.is_a?(Lic::Plugin::API::Source)
      # because that _could_ trip the autoload, and if there are unresolved
      # gems at that time, this method could be called inside another require,
      # thus raising with that constant being undefined. Better to check a method
      if source.respond_to?(:path) || (source.respond_to?(:lic_plugin_api_source?) && source.lic_plugin_api_source?)
    ...
    if method_defined?(:extension_dir)
      ...
      def extension_dir
        @lic_extension_dir ||= if source.respond_to?(:extension_dir_name)
    ...
    def git_version
      return unless loaded_from && source.is_a?(Lic::Source::Git)
      " #{source.revision[0..6]}"
    end
