...
module Lic
  class CLI::Outdated
    ...
    def run
      ...
      gems.each do |gem_name|
        Lic::CLI::Common.select_spec(gem_name)
      end

      Lic.definition.validate_runtime!
      current_specs = Lic.ui.silence { Lic.definition.resolve }
      current_dependencies = {}
      Lic.ui.silence do
        Lic.load.dependencies.each do |dep|
      ...
      definition = if gems.empty? && sources.empty?
        ...
        Lic.definition(:gems => gems, :sources => sources)
      end

      Lic::CLI::Common.configure_gem_version_promoter(
        Lic.definition,
        options
      )
      ...
      strict = options["filter-strict"] ||
        Lic::CLI::Common.patch_level_options(options).any?
      ...
      if options[:parseable]
        Lic.ui.silence(&definition_resolution)
      else
      ...
      if outdated_gems_list.empty?
        ...
        unless options[:parseable]
          if options[:pre]
            Lic.ui.info "Outdated gems included in the lic (including " \
              "pre-releases):"
          else
            Lic.ui.info "Outdated gems included in the lic:"
          end
        end
        ...
        if options_include_groups.any?
          ...
          [nil, ordered_groups].flatten.each do |groups|
            ...
            unless options[:parseable]
              if groups
                Lic.ui.info "===== Group #{groups} ====="
              else
                Lic.ui.info "===== Without group ====="
              end
            end
  ...
  private
    ...
    def display_nothing_outdated_message(filter_options_patch)
      unless options[:parseable]
        if filter_options_patch.any?
          ...
          Lic.ui.info "No #{display} updates to display.\n"
        else
          Lic.ui.info "Bundle up to date!\n"
        end
      end
    end
    ...
    def print_gem(current_spec, active_spec, dependency, groups, options_include_groups)
      ...
      spec_version += " (from #{active_spec.loaded_from})" if Lic.ui.debug? && active_spec.loaded_from
      ...
      Lic.ui.info output_message.rstrip
    end

    def check_for_deployment_mode
      return unless Lic.frozen_lic?
      ...
      raise ProductionError, "You are trying to check outdated gems in " \
        "deployment mode. Run `lic outdated` elsewhere.\n" \
        "\nIf this is a development machine, remove the " \
        "#{Lic.default_gemfile} freeze" \
