...
module Lic
  class Settings
    autoload :Mirror,  "lic/mirror"
    autoload :Mirrors, "lic/mirror"
    autoload :Validator, "lic/settings/validator"

    BOOL_KEYS = %w[
      allow_lic_dependency_conflicts
    ...
    def set_command_option(key, value)
      if Lic.feature_flag.forget_cli_options?
        temporary(key => value)
        value
      else
        command = if value.nil?
          "lic config --delete #{key}"
        else
          "lic config #{key} #{Array(value).join(":")}"
        end

        Lic::SharedHelpers.major_deprecation 2,\
          "flags passed to commands " \
          "will no longer be automatically remembered. Instead please set flags " \
          "you want remembered between commands using `lic config " \
    ...
    # for legacy reasons, in Lic 1, the ruby scope isnt appended when the setting comes from ENV or the global config,
    # nor do we respect :disable_shared_gems
    def path
      key  = key_for(:path)
      path = ENV[key] || @global_config[key]
      if path && !@temporary.key?(key) && !@local_config.key?(key)
        return Path.new(path, Lic.feature_flag.global_path_appends_ruby_scope?, false, false)
      end

      system_path = self["path.system"] || (self[:disable_shared_gems] == false)
      Path.new(self[:path], true, system_path, Lic.feature_flag.default_install_uses_path?)
    end

    Path = Struct.new(:explicit_path, :append_ruby_scope, :system_path, :default_install_uses_path) do
      def path
        path = base_path
        path = File.join(path, Lic.ruby_scope) if append_ruby_scope && !use_system_gems?
        path
      end
      ...
      def base_path
        path = explicit_path
        path ||= ".lic" unless use_system_gems?
        path ||= Lic.rubygems.gem_dir
        path
      end

      def base_path_relative_to_pwd
        base_path = Pathname.new(self.base_path)
        expanded_base_path = base_path.expand_path(Lic.root)
      ...
      def validate!
        ...
        path = Lic.settings.pretty_values_for(:path)
        path.unshift(nil, "path:") unless path.empty?
        system_path = Lic.settings.pretty_values_for("path.system")
        system_path.unshift(nil, "path.system:") unless system_path.empty?
        disable_shared_gems = Lic.settings.pretty_values_for(:disable_shared_gems)
  ...
  private
    ...
    def global_config_file
      if ENV["LIC_CONFIG"] && !ENV["LIC_CONFIG"].empty?
        Pathname.new(ENV["LIC_CONFIG"])
      else
        begin
          Lic.user_lic_path("config")
