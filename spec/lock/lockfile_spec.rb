...
RSpec.describe "the lockfile format", :lic => "2" do
  ...
  it "does not update the lockfile's lic version if nothing changed during lic install" do
  ...
  it "outputs a warning if the current is older than lockfile's lic version" do
    ...
    expect(last_command.lic_err).to include warning_message
  ...
  it "errors if the current is a major version older than lockfile's lic version" do
    ...
    expect(last_command.lic_err).to include("You must use Lic 9999999 or greater with this lockfile.")
  end
  ...
  it "generates a simple lockfile for a single pinned source, gem with a version requirement", :lic => "< 2" do
    git = build_git "foo"
