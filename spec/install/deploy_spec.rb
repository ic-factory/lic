...
RSpec.describe "install with --deployment or --frozen" do
  ...
  context "with CLI flags", :lic => "< 2" do
    ...
    it "disallows --deployment --path --system" do
      lic "install --deployment --path . --system"
  ...
  it "still works if you are not in the app directory and specify --gemfile" do
    ...
    Dir.chdir tmp do
      ...
      lic! :install,
        forgotten_command_line_options(:gemfile => "#{tmp}/licd_app/Gemfile",
                                       :deployment => true,
                                       :path => "vendor/lic")
    end
    expect(the_lic).to include_gems "rack 1.0"
  ...
  it "works if you exclude a group with a git gem" do
    ...
    G
    lic :install
    lic! :install, forgotten_command_line_options(:deployment => true, :without => "test")
  end

  it "works when you lic exec lic", :ruby_repo do
    lic :install
    lic "install --deployment"
    lic! "exec lic check"
  end
  ...
  it "works when there are credentials in the source URL" do
    ...
    lic! :install, forgotten_command_line_options(:deployment => true).merge(:artifice => "endpoint_strict_basic_authentication")
  end
  ...
  describe "with an existing lockfile" do
    ...
    it "works with the --deployment flag if you didn't change anything", :lic => "< 2" do
      lic! "install --deployment"
    end

    it "works with the --frozen flag if you didn't change anything", :lic => "< 2" do
      lic! "install --frozen"
    end

    it "works with LIC_FROZEN if you didn't change anything" do
      lic! :install, :env => { "LIC_FROZEN" => "true" }
    end
    ...
    it "works if a path gem is missing but is in a without group" do
      ...
      G
      expect(the_lic).to include_gems "path_gem 1.0"
      FileUtils.rm_r lib_path("path_gem-1.0")

      lic! :install, forgotten_command_line_options(:path => ".lic", :without => "development", :deployment => true).merge(:env => { :DEBUG => "1" })
      run! "puts :WIN"
    ...
    it "explodes if a path gem is missing" do
      ...
      lic :install, forgotten_command_line_options(:path => ".lic", :deployment => true)
      expect(out).to include("The path `#{lib_path("path_gem-1.0")}` does not exist.")
    end

    it "can have --frozen set via an environment variable", :lic => "< 2" do
      gemfile <<-G
        source "file://#{gem_repo1}"
        gem "rack"
        gem "rack-obama"
    ...
    it "explodes with the --frozen flag if you make a change and don't check in the lockfile", :lic => "< 2" do
      gemfile <<-G
        source "file://#{gem_repo1}"
        gem "rack"
        gem "rack-obama", "1.1"
      ...
      lic :install, forgotten_command_line_options(:frozen => true)
      expect(out).to include("deployment mode")
      expect(out).to include("You have added to the Gemfile")
    ...
    context "when replacing a host with the same host with credentials" do
      let(:success_message) do
        if Lic::VERSION.split(".", 2).first == "1"
      ...
      context "when allow_deployment_source_credential_changes is true" do
        before { lic! "config allow_deployment_source_credential_changes true" }
      ...
      context "when allow_deployment_source_credential_changes is false" do
        before { lic! "config allow_deployment_source_credential_changes false" }
      ...
      context "when LIC_ALLOW_DEPLOYMENT_SOURCE_CREDENTIAL_CHANGES env var is true" do
        before { ENV["LIC_ALLOW_DEPLOYMENT_SOURCE_CREDENTIAL_CHANGES"] = "true" }
      ...
      context "when LIC_ALLOW_DEPLOYMENT_SOURCE_CREDENTIAL_CHANGES env var is false" do
        before { ENV["LIC_ALLOW_DEPLOYMENT_SOURCE_CREDENTIAL_CHANGES"] = "false" }
    ...
    it "remembers that the lic is frozen at runtime" do
      lic! :lock
...
* rack
  ...
  context "with path in Gemfile and packed" do
    it "works fine after lic package and lic install --local" do
      ...
      expect(licd_app("vendor/cache/foo")).to be_directory

      lic! "install --local"
      expect(out).to include("Updating files in vendor/cache")

      simulate_new_machine
      lic! "install --verbose", forgotten_command_line_options(:deployment => true)
