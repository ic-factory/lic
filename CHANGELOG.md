...
Features:
  ...
  - Add `--only-explicit` option to `lic outdated` to list only outdated gems in the Gemfile ([#5366](https://github.com/lic/lic/issues/5366), @peret)
...
Bugfixes:
  ...
  - print a helpful error when `lic exec`ing to a gem that isn't included in the lic ([#5487](https://github.com/lic/lic/issues/5487), @segiddins)
...
Features:
  ...
  - improve error message when sources are not absolute URIs ([#3925](https://github.com/lic/lic/issues/3925), @agis-)
...
Bugfixes:
  ...
  - Recognize `:mri_22` and `:mingw_22`, rather than just `:ruby_22` ([#3328](https://github.com/lic/lic/issues/3328), @myabc)
...
Bugfixes:
  ...
  - Fix --deployment with source blocks and non-alphabetical gems ([#3224](https://github.com/lic/lic/issues/3224), @TimMoore)
...
Features:
  ...
  - use RUBYLIB instead of RUBYOPT for better Windows support ([#2536](https://github.com/lic/lic/issues/2536), @equinux)
...
Security:
  ...
  - add `--trust-policy` to `install`, like `gem install -P` (@CosmicCat, [#2293](https://github.com/lic/lic/issues/2293))

Features:

  - optimize resolver when too new of a gem is already activated (@rykov, [#2248](https://github.com/lic/lic/issues/2248))
  ...
  - add `licenses` command (@bryanwoods, [#1898](https://github.com/lic/lic/issues/1898))
  - sort output from `outdated` (@richardkmichael, [#1896](https://github.com/lic/lic/issues/1896))
  - add a .travis.yml to `gem -t` (@ndbroadbent, [#2143](https://github.com/lic/lic/issues/2143))
  - inform users when the resolver starts
  - disable reverse DNS to speed up API requests (@raggi)

Bugfixes:

  - raise errors while requiring dashed gems ([#1807](https://github.com/lic/lic/issues/1807))
  - quote the Lic path on Windows (@jgeiger, [#1862](https://github.com/lic/lic/issues/1862), [#1856](https://github.com/lic/lic/issues/1856))
  - load gemspecs containing unicode (@gaffneyc, [#2301](https://github.com/lic/lic/issues/2301))
  - support any ruby version in --standalone
  - resolve some ruby -w warnings (@chastell, [#2193](https://github.com/lic/lic/issues/2193))
...
Bugfixes:
  ...
  - stubs for gems with dev deps no longer cause exceptions ([#2272](https://github.com/lic/lic/issues/2272))
...
Features:
  ...
  - `lic gem --edit` will open the new gemspec (@ndbroadbent)
  - `lic gem --test rspec` now makes working tests (@tricknotes)
  - `lic env` prints info about lic's environment (@peeja)
  - add `LIC_IGNORE_CONFIG` environment variable support (@richo)

Bugfixes:

  - don't overwrite custom binstubs during `install --binstubs`
  - don't throw an exception if `binstubs` gem doesn't exist
  - `lic config` now works in directories without a Gemfile
...
Features:
  ...
  - add `--dry-run` to `lic clean` (@wfarr, [#2237](https://github.com/lic/lic/issues/2237))

Bugfixes:

  - don't skip writing binstubs when doing `lic install`
...
Features:
  ...
  - `lic binstubs <gem>` to setup individual binstubs
  - `lic install --binstubs ""` will remove binstubs option
  - `lic clean --dry-run` will print out gems instead of removing them

Bugfixes:

  - Avoid stack traces when Ctrl+C during lic command (@mitchellh)
...
Bugfixes:
  ...
  - `outdated` exits non-zero if outdated gems found (@rohit, [#2021](https://github.com/lic/lic/issues/2021))
...
Features:
  ...
  - `config` expands local overrides like `local.rack .` (@gkop, [#2205](https://github.com/lic/lic/issues/2205))
  - `gem` generates files correctly for names like `jquery-rails` (@banyan, [#2201](https://github.com/lic/lic/issues/2201))
...
Bugfixes:
  ...
  - caching git gems now caches specs, fixing e.g. git ls-files (@bison, [#2039](https://github.com/lic/lic/issues/2039))
  - `show GEM` now warns if the directory has been deleted (@rohit, [#2070](https://github.com/lic/lic/issues/2070))
...
Features:
  ...
  - significant performance improvements (@cheald, [#2181](https://github.com/lic/lic/issues/2181))
...
Bugfixes:
  ...
  - `install` installs both rake rake-built gems at once (@crowbot, [#2107](https://github.com/lic/lic/issues/2107))
  ...
  - raise LoadErrors correctly during Lic.require (@Empact)
  - do not swallow --verbose on `lic exec` (@sol, [#2102](https://github.com/lic/lic/issues/2102))
...
Bugfixes:
  ...
  - `lic viz` works with git gems again (@hirochachacha)
...
Bugfixes:
  ...
  - `lic clean` now works with LIC_WITHOUT groups again
...
Features:
  ...
  - `check` now has a `--dry-run` option (@svenfuchs, [#1811](https://github.com/lic/lic/issues/1811))
  - loosen ruby directive for engines
  - prune git/path directories inside vendor/cache (@josevalim, [#1988](https://github.com/lic/lic/issues/1988))
  - update vendored thor to 0.15.2 (@sferik)
  - add .txt to LICENSE (@postmodern, [#2001](https://github.com/lic/lic/issues/2001))
  - add `config disable_local_branch_check` (@josevalim, [#1985](https://github.com/lic/lic/issues/1985))
  - fall back on the full index when experiencing syck errors ([#1419](https://github.com/lic/lic/issues/1419))
  - handle syntax errors in Ruby gemspecs ([#1974](https://github.com/lic/lic/issues/1974))

Bugfixes:

  - fix `pack`/`cache` with `--all` (@josevalim, [#1989](https://github.com/lic/lic/issues/1989))
  - don't display warning message when `cache_all` is set
  - check for `nil` PATH ([#2006](https://github.com/lic/lic/issues/2006))
  - Always try to keep original GEM_PATH (@drogus, [#1920](https://github.com/lic/lic/issues/1920))

## 1.2.0.pre.1 (May 27, 2012)

Features:

  - Git gems import submodules of submodules recursively (@nwwatson, [#1935](https://github.com/lic/lic/issues/1935))

Bugfixes:

  - Exit from `check` with a non-zero status when frozen with no lock
  - Use `latest_release` in Capistrano and Vlad integration ([#1264](https://github.com/lic/lic/issues/1264))
...
Documentation:
  ...
  - When OpenSSL is missing, provide instructions for fixing ([#1776](https://github.com/lic/lic/issues/1776) etc.)
  - Unknown exceptions now link to ISSUES for help instead of a new ticket
  - Correct inline help for `clean --force` (@dougbarth, [#1911](https://github.com/lic/lic/issues/1911))

## 1.2.0.pre (May 4, 2012)

Features:

  - lic package now accepts --all to package git and path dependencies
  - lic config now accepts --local, --global and --delete options
  - It is possible to override a git repository via configuration.
    ...
    it to use a local repo with `lic config local.rack ~/path/to/rack`
  - Cache gemspec loads for performance (@dekellum, [#1635](https://github.com/lic/lic/issues/1635))
  - add --full-index flag to `lic update` (@fluxx, [#1829](https://github.com/lic/lic/issues/1829))
  - add --quiet flag to `lic update` (@nashby, [#1654](https://github.com/lic/lic/issues/1654))
  - Add Lic::GemHelper.gemspec (@knu, [#1637](https://github.com/lic/lic/issues/1637))
  - Graceful handling of Gemfile syntax errors (@koraktor, [#1661](https://github.com/lic/lic/issues/1661))
  - `lic platform` command
  - add ruby to DSL, to specify version of ruby
  - error out if the ruby version doesn't match

Performance:

  - lic exec shouldn't run Lic.setup just setting the right rubyopts options is enough (@spastorino, [#1598](https://github.com/lic/lic/issues/1598))

Bugfixes:

  - Avoid passing RUBYOPT changes in with_clean_env block (@eric1234, [#1604](https://github.com/lic/lic/issues/1604))
  - Use the same ruby to run subprocesses as is running rake (@brixen)

Documentation:

  - Add :github documentation in DSL (@zofrex, [#1848](https://github.com/lic/lic/issues/1848), [#1851](https://github.com/lic/lic/issues/1851), [#1852](https://github.com/lic/lic/issues/1852))
  - Add docs for the --no-cache option (@fluxx, [#1796](https://github.com/lic/lic/issues/1796))
  - Add basic documentation for bin_path and lic_path (@radar)
  - Add documentation for the run method in Lic::Installer
...
Bugfixes:
  ...
  - escape the lic root path (@tenderlove, [#1789](https://github.com/lic/lic/issues/1789))

## 1.1.2 (March 20, 2012)

Bugfixes:

  - Fix --deployment for multiple PATH sections of the same source ([#1782](https://github.com/lic/lic/issues/1782))
...
Bugfixes:
  ...
  - Use $\ so `lic gem` gemspecs work on Windows (@postmodern)
  - URI-encode gem names for dependency API (@rohit, [#1672](https://github.com/lic/lic/issues/1672))
  - Fix `cache` edge case in rubygems 1.3.7 ([#1202](https://github.com/lic/lic/issues/1202))

Performance:

  - Reduce invocation of git ls-files in `lic gem` gemspecs (@knu)

## 1.1.0 (Mar 7, 2012)

Bugfixes:

  - Clean up corrupted lockfiles on lic installs
...
Bugfixes:
  ...
  - Load gemspecs from git even when a released gem has the same version ([#1609](https://github.com/lic/lic/issues/1609))
  - Declare an accurate Ruby version requirement of 1.8.7 or newer ([#1619](https://github.com/lic/lic/issues/1619))
  - handle gemspec development dependencies correctly (@raggi, [#1639](https://github.com/lic/lic/issues/1639))
  - Avoid passing RUBYOPT changes in with_clean_env block. (eric1234, [#1604](https://github.com/lic/lic/issues/1604))
...
Bugfixes:
  ...
  - Fix performance regression from 1.0 (@spastorino, [#1511](https://github.com/lic/lic/issues/1511), [#1591](https://github.com/lic/lic/issues/1591), [#1592](https://github.com/lic/lic/issues/1592))
  - Load gems correctly when GEM_HOME is blank
  - Refresh gems so Lic works from inside a lic
  - Handle empty .lic/config files without an error
...
Features:
  ...
  - `lic viz` has the option to output a DOT file instead of a PNG (@hirochachacha, [#683](https://github.com/lic/lic/issues/683))

Bugfixes:

  - Ensure binstubs generated when using --standalone point to the standalonde lic (@cowboyd, [#1588](https://github.com/lic/lic/issues/1588))
  - fix `lic viz` (@hirochachacha, [#1586](https://github.com/lic/lic/issues/1586))

## 1.1.rc.3 (Dec 8, 2011)

Bugfixes:

  - fix relative_path so it checks Lic.root is actually in the beginning of the path ([#1582](https://github.com/lic/lic/issues/1582))
  - fix lic outdated doesn't list all gems (@joelmoss, [#1521](https://github.com/lic/lic/issues/1521))

## 1.1.rc.2 (Dec 6, 2011)

Features:

  - Added README.md to `newgem` (@ognevsky, [#1574](https://github.com/lic/lic/issues/1574))
  - Added LICENSE (MIT) to newgem (@ognevsky, [#1571](https://github.com/lic/lic/issues/1571))

Bugfixes:

  - only auto-namespace requires for implied requires ([#1531](https://github.com/lic/lic/issues/1531))
  - fix lic clean output for git repos ([#1473](https://github.com/lic/lic/issues/1473))
  - use Gem.bindir for lic clean ([#1544](https://github.com/lic/lic/issues/1544), [#1532](https://github.com/lic/lic/issues/1532))
  - use `Gem.load_env_plugins` instead of `Gem.load_env_plugins` ([#1500](https://github.com/lic/lic/issues/1500), [#1543](https://github.com/lic/lic/issues/1543))
  - differentiate Ruby 2.0 (trunk) from Ruby 1.9 (@tenderlove, [#1539](https://github.com/lic/lic/issues/1539))
  - `lic clean` handles 7 length git hash for lic clean ([#1490](https://github.com/lic/lic/issues/1490), [#1491](https://github.com/lic/lic/issues/1491))
  - fix Psych loading issues
  - Search $PATH for a binary rather than shelling out to `which` (@tenderlove, [#1573](https://github.com/lic/lic/issues/1573))
  - do not clear RG cache unless we actually modify GEM_PATH and GEM_HOME- use `Gem.load_env_plugins` instead of `Gem.load_env_plugins` ([#1500](https://github.com/lic/lic/issues/1500), [#1543](https://github.com/lic/lic/issues/1543))
  - `newgem` now uses https://rubygems.org ([#1562](https://github.com/lic/lic/issues/1562))
  - `lic init` now uses https://rubygems.org (@jjb, [#1522](https://github.com/lic/lic/issues/1522))
  - `lic install/update` does not autoclean when using --path for semver

Documentation:

  - added documentation for --shebang option for `lic install` (@lunks, [#1475](https://github.com/lic/lic/issues/1475), [#1558](https://github.com/lic/lic/issues/1558))

## 1.1.rc (Oct 3, 2011)

Features:

  - add `--shebang` option to lic install (@bensie, [#1467](https://github.com/lic/lic/issues/1467))
  - build passes on ruby 1.9.3rc1 ([#1458](https://github.com/lic/lic/issues/1458), [#1469](https://github.com/lic/lic/issues/1469))
  - hide basic auth credentials for custom sources ([#1440](https://github.com/lic/lic/issues/1440), [#1463](https://github.com/lic/lic/issues/1463))

Bugfixes:

  - fix index search result caching ([#1446](https://github.com/lic/lic/issues/1446), [#1466](https://github.com/lic/lic/issues/1466))
  - fix fetcher prints multiple times during install ([#1445](https://github.com/lic/lic/issues/1445), [#1462](https://github.com/lic/lic/issues/1462))
  - don't mention API errors from non-rubygems.org sources
  - fix autoclean so it doesn't remove bins that are used ([#1459](https://github.com/lic/lic/issues/1459), [#1460](https://github.com/lic/lic/issues/1460))

Documentation:

  - add :require => [...] to the gemfile(5) manpage (@nono, [#1468](https://github.com/lic/lic/issues/1468))
...
Features:
  ...
  - `clean` will be automatically run after lic install and update when using `--path` ([#1420](https://github.com/lic/lic/issues/1420), [#1425](https://github.com/lic/lic/issues/1425))
  - `clean` now takes a `--force` option ([#1247](https://github.com/lic/lic/issues/1247), [#1426](https://github.com/lic/lic/issues/1426))
  - `clean` will clean up cached git dirs in lic clean ([#1390](https://github.com/lic/lic/issues/1390))
  - remove deprecations from DSL ([#1119](https://github.com/lic/lic/issues/1119))
  - autorequire tries directories for gems with dashed names ([#1205](https://github.com/lic/lic/issues/1205))
  - adds a `--paths` flag to `lic show` to list all the paths of licd gems (@tiegz, [#1360](https://github.com/lic/lic/issues/1360))
  - load rubygems plugins in the lic binary (@tpope, [#1364](https://github.com/lic/lic/issues/1364))
  - make `--standalone` respect `--path` (@cowboyd, [#1361](https://github.com/lic/lic/issues/1361))

Bugfixes:

  - Fix `clean` to handle nested gems in a git repo ([#1329](https://github.com/lic/lic/issues/1329))
  - Fix conflict from revert of benchmark tool (@boffbowsh, [#1355](https://github.com/lic/lic/issues/1355))
  - Fix fatal error when unable to connect to gem source ([#1269](https://github.com/lic/lic/issues/1269))
  - Fix `outdated` to find pre-release gems that are installed. ([#1359](https://github.com/lic/lic/issues/1359))
  - Fix color for ui. ([#1374](https://github.com/lic/lic/issues/1374))
  - Fix installing to user-owned system gems on OS X
  - Fix caching issue in the resolver ([#1353](https://github.com/lic/lic/issues/1353), [#1421](https://github.com/lic/lic/issues/1421))
...
Bugfixes:
  ...
  - Fix `lic check` to not print fatal error message (@cldwalker, [#1347](https://github.com/lic/lic/issues/1347))
  - Fix require_sudo when Gem.bindir isn't writeable ([#1352](https://github.com/lic/lic/issues/1352))
  - Fix not asking Gemcutter API for dependency chain of git gems in --deployment ([#1254](https://github.com/lic/lic/issues/1254))
  - Fix `install --binstubs` when using --path ([#1332](https://github.com/lic/lic/issues/1332))

## 1.1.pre.7 (Aug 8, 2011)

Bugfixes:

  - Fixed invalid byte sequence error while installing gem on Ruby 1.9 ([#1341](https://github.com/lic/lic/issues/1341))
...
Bugfixes:
  ...
  - Fix cross repository dependencies ([#1138](https://github.com/lic/lic/issues/1138))
  - Fix git dependency fetching from API endpoint ([#1254](https://github.com/lic/lic/issues/1254))
  - Fixes for lic outdated (@joelmoss, [#1238](https://github.com/lic/lic/issues/1238))
  - Fix lic standalone when using the endpoint ([#1240](https://github.com/lic/lic/issues/1240))

Features:

  - Implement `to_ary` to avoid calls to method_missing (@tenderlove, [#1274](https://github.com/lic/lic/issues/1274))
  - lic clean removes old .gem files (@cldwalker, [#1293](https://github.com/lic/lic/issues/1293))
  - Correcly identify missing child dependency in error message
  - Run pre-install, post-build, and post-install gem hooks for git gems (@warhammerkid, [#1120](https://github.com/lic/lic/issues/1120))
  - create Gemfile.lock for empty Gemfile ([#1218](https://github.com/lic/lic/issues/1218))

## 1.1.pre.5 (June 11, 2011)

Bugfixes:

  - Fix LazySpecification on Ruby 1.9 (@dpiddy, [#1232](https://github.com/lic/lic/issues/1232))
  - Fix HTTP proxy support (@leobessa, [#878](https://github.com/lic/lic/issues/878))

Features:

  - Speed up `install --deployment` by using the API endpoint
  - Support Basic HTTP Auth for the API endpoint (@dpiddy, [#1229](https://github.com/lic/lic/issues/1229))
  ...
  - `outdated` command shows outdated gems (@joelmoss, [#1130](https://github.com/lic/lic/issues/1130))
  - Print gem post install messages (@csquared, [#1155](https://github.com/lic/lic/issues/1155))
  - Reduce memory use by removing Specification.new inside method_missing (@tenderlove, [#1222](https://github.com/lic/lic/issues/1222))
  - Allow `check --path`
...
Features:
  ...
  - Add `lic outdated` to show outdated gems (@joelmoss)
  - Remove LIC_* from `Lic.with_clean_env` (@wuputah)
  - Add Lic.clean_system, and clean_exec (@wuputah)
  - Use git config for gem author name and email (@krekoten)

Bugfixes:

  - Fix error calling Lic.rubygems.gem_path
...
Features:
  ...
  - Add lic clean. Removes unused gems from --path directory
  - Initial Gemcutter Endpoint API work, BAI Fetching source index
  - Added lic install --standalone
...
Removed:
  ...
  - Removed lic lock
  - Removed lic install <path>
  - Removed lic install --production
  - Removed lic install --disable-shared-gems
...
Features:
  ...
  - Add platform :maglev (@timfel, [#1444](https://github.com/lic/lic/issues/1444))
...
Features:
  ...
  - Rescue interrupts to `lic` while loading lic.rb ([#1395](https://github.com/lic/lic/issues/1395))
  - Allow clearing without groups by passing `--without ''` ([#1259](https://github.com/lic/lic/issues/1259))

Bugfixes:

  - Manually sort requirements in the lockfile ([#1375](https://github.com/lic/lic/issues/1375))
  - Remove several warnings generated by ruby -w (@stephencelis)
  - Handle trailing slashes on names passed to `gem` ([#1372](https://github.com/lic/lic/issues/1372))
  - Name modules for gems like 'test-foo_bar' correctly ([#1303](https://github.com/lic/lic/issues/1303))
  - Don't require Psych if Syck is already loaded ([#1239](https://github.com/lic/lic/issues/1239))
...
Features:
  ...
  - Report gem installation failures clearly (@rwilcox, [#1380](https://github.com/lic/lic/issues/1380))
  - Useful error for cap and vlad on first deploy (@nexmat, @kirs)

Bugfixes:

  - `exec` now works when the command contains 'exec'
  - Only touch lock after changes on Windows (@robertwahler, [#1358](https://github.com/lic/lic/issues/1358))
  - Keep load paths when #setup is called multiple times (@radsaq, [#1379](https://github.com/lic/lic/issues/1379))
...
Bugfixes:
  ...
  - Fixes rake 0.9.x warning (@mtylty, [#1333](https://github.com/lic/lic/issues/1333))
  - Fix `lic cache` again for rubygems 1.3.x

Features:

  - Run the lic install earlier in a Capistrano deployment (@cgriego, [#1300](https://github.com/lic/lic/issues/1300))
  - Support hidden gemspec (@trans, @cldwalker, [#827](https://github.com/lic/lic/issues/827))
  - Make fetch_specs faster (@zeha, [#1294](https://github.com/lic/lic/issues/1294))
  - Allow overriding development deps loaded by #gemspec (@lgierth, [#1245](https://github.com/lic/lic/issues/1245))

## 1.0.17 (August 8, 2011)

Bugfixes:

  - Fix rake issues with rubygems 1.3.x ([#1342](https://github.com/lic/lic/issues/1342))
  - Fixed invalid byte sequence error while installing gem on Ruby 1.9 ([#1341](https://github.com/lic/lic/issues/1341))
...
Features:
  ...
  - Performance fix for MRI 1.9 (@efficientcloud, [#1288](https://github.com/lic/lic/issues/1288))
  - Shortcuts (like `lic i`) for all commands (@amatsuda)
  - Correcly identify missing child dependency in error message

Bugfixes:

  - Allow Windows network share paths with forward slashes (@mtscout6, [#1253](https://github.com/lic/lic/issues/1253))
  - Check for rubygems.org credentials so `rake release` doesn't hang ([#980](https://github.com/lic/lic/issues/980))
  - Find cached prerelease gems on rubygems 1.3.x (@dburt, [#1202](https://github.com/lic/lic/issues/1202))
  - Fix `lic install --without` on kiji (@tmm1, [#1287](https://github.com/lic/lic/issues/1287))
  - Get rid of warning in ruby 1.9.3 (@smartinez87, [#1231](https://github.com/lic/lic/issues/1231))

Documentation:

  - Documentation for `gem ..., :require => false` (@kmayer, [#1292](https://github.com/lic/lic/issues/1292))
  - Gems provide "executables", they are rarely also binaries (@fxn, [#1242](https://github.com/lic/lic/issues/1242))
...
Features:
  ...
  - Include gem rake tasks with "require 'lic/gem_tasks" (@indirect)
...
Bugfixes:
  ...
  - Install gems that contain 'lic', e.g. guard-lic (@hone)
...
Bugfixes:
  ...
  - Have cap run lic install from release_path (@martinjagusch)
...
Features:
  ...
  - Honor :lic_cmd in cap `rake` command (@voidlock, @cgriego)
...
Bugfixes:
  ...
  - Fix --no-prune option for `lic install` (@cmeiklejohn)
...
Bugfixes:
  ...
  - Fix a bug where Lic.require could remove gems from the load
...
Features:
  ...
  - Add --local option to `lic update`
  - Ignore Gemfile.lock in newly generated gems
  - Use `less` as help pager instead of `more`
  - Run `lic exec rake` instead of `rake` in Capistrano tasks

Bugfixes:

  - Fix --no-cache option for `lic install`
  - Allow Vlad deploys to work without Capistrano gem installed
  - Fix group arguments to `lic console`
...
Bugfixes:
  ...
  - Remove Lic version from the lockfile because it broke
...
Bugfixes:
  ...
  - Allow git gems on other platforms while installing ([#579](https://github.com/lic/lic/issues/579))
...
Bugfixes:
  ...
  - Expand relative :paths from Lic.root (eg ./foogem)
  - Allow git gems in --without groups while --frozen
  - Allow gem :ref to be a symbol as well as a string
  - Fix exception when Gemfile needs a newer Lic version
  - Explanation when the current Lic version conflicts
  - Explicit error message if Gemfile needs newer Lic
  - Ignore an empty string LIC_GEMFILE
...
Bugfixes:
  ...
  - `lic update` now works with :git + :tag updates
...
Features:
  ...
  - Vlad deployment recipe, `require 'lic/vlad'`
  - Prettier lic graphs
  - Improved gem skeleton for `lic gem`
...
Features:
  ...
  - You can now define `:lic_cmd` in the capistrano task
...
Bugfixes:
  ...
  - Update the Gemfile template to reference rubygems.org instead
    of :gemcutter
  - lic exec should exit with a non zero exit code when the gem
...
Features:
  ...
  - `lic gem NAME` command to generate a new gem with Gemfile
  - Bundle config file location can be specified by LIC_APP_CONFIG
  - Add --frozen to disable updating the Gemfile.lock at runtime
    (default with --deployment)
  - Basic Capistrano task now added as 'lic/capistrano'

Bugfixes:

  - Multiple lic process no longer share a tmp directory
  - `lic update GEM` always updates dependencies of GEM as well
  - Deleting the cache directory no longer causes errors
  - Moving the lic after installation no longer causes git errors
...
Features:
  ...
  - Add --gemfile option to `lic check`
  - Reduce memory usage of `lic install` by 2-4x
  - Improve message from `lic check` under various conditions
...
## 1.0.0.rc.2 (July 29, 2010)
  ...
  - `lic install path` was causing confusion, so we now print
    a clarifying warning. The preferred way to install to a path
    (which will not print the warning) is
    `lic install --path path/to/install`.
  - `lic install --system` installs to the default system
    location ($LIC_PATH or $GEM_HOME) even if you previously
    used `lic install --path`
  ...
  - Add a `--production` option to `lic install`:
    - by default, installs to `vendor/lic`. This can be
      overridden with the `--path` option
    - uses `--local` if `vendor/cache` is found. This will
      guarantee that Lic does not attempt to connect to
...
been checked out. Please run lic install". As far as we
can tell, these problems have two possible causes:

1. `lic install ~/.lic` in one user, but actually running
...
Capistrano, symlink release_path/current/vendor/lic to
release_path/shared/lic. This will keep deployments
snappy while maintaining the benefits of clean, deploy-time
isolation.

## 1.0.0.rc.1 (July 26, 2010)

  - Fixed a bug with `lic install` on multiple machines and git

## 1.0.0.beta.10 (July 25, 2010)

  - Last release before 1.0.0.rc.1
  - Added :mri as a valid platform (platforms :mri { gem "ruby-debug" })
  - Fix `lic install` immediately after modifying the :submodule option
  - Don't write to Gemfile.lock if nothing has changed, fixing situations
    where lic install was run with a different user than the app
    itself
  - Fix a bug where other platforms were being wiped on `lic update`
  - Don't ask for root password on `lic install` if not needed
  - Avoid setting `$GEM_HOME` where not needed
  - First solid pass of `lic config`
  - Add build options
    - `lic config build.mysql --with-mysql-config=/path/to/config`

## 1.0.0.beta.9 (July 21, 2010)

  - Fix install failure when switching from a path to git source
  - Fix `lic exec lic *` in a lic with --disable-shared-gems
  - Fix `lic *` from inside a lic with --disable-shared-gem
...
## 1.0.0.beta.7 (July 20, 2010, yanked)
  ...
  - Running `lic install` twice in a row with a git source always crashed

## 1.0.0.beta.6 (July 20, 2010, yanked)

  - Create executables with lic install --binstubs
  ...
  - Improve performance of lic install if the git gem was already checked out,
    and the revision being used already exists locally
  - Fix lic show lic in some cases
  - Fix bugs with lic update
  ...
  - Fix a bug where require errors were being swallowed during Lic.require

## 1.0.0.beta.1

  - No `lic lock` command. Locking happens automatically on install or update
  - No .lic/environment.rb. Require 'lic/setup' instead.
  - $LIC_HOME defaults to $GEM_HOME instead of ~/.lic
...
Features:
  ...
  - limit the licd version of lic to the same as the one installing
  ...
  - raise Lic::GemNotFound instead of calling exit! inside library code
...
Bugfixes:
  ...
  - do not monkeypatch Specification#load_paths twice when inside a lic
...
Features:
  ...
  - handle Polyglot's changes to Kernel#require with Lic::ENV_LOADED ([#287](https://github.com/lic/lic/issues/287))
  - remove .gem files generated after installing a gem from a :path ([#286](https://github.com/lic/lic/issues/286))
  - improve install/lock messaging ([#284](https://github.com/lic/lic/issues/284))

Bugfixes:

  - ignore cached gems that are for another platform ([#288](https://github.com/lic/lic/issues/288))
  - install Windows gems that have no architecture set, like rcov ([#277](https://github.com/lic/lic/issues/277))
  - exec command while locked now includes the lic lib in $LOAD_PATH ([#293](https://github.com/lic/lic/issues/293))
  - fix the `rake install` task
  - add GemspecError so it can be raised without (further) error ([#292](https://github.com/lic/lic/issues/292))
  - create a parent directory before cloning for git 1.5 compatibility ([#285](https://github.com/lic/lic/issues/285))
...
Features:
  ...
  - no backtraces when calling Lic.setup if gems are missing
  - no backtraces when trying to exec a file without the executable bit

Bugfixes:

  - fix infinite recursion in Lic.setup after loading a licd Lic gem
...
Features:
  ...
  - suggest `lic install --relock` when the Gemfile has changed ([#272](https://github.com/lic/lic/issues/272))
  - source support for Rubygems servers without prerelease gem indexes ([#262](https://github.com/lic/lic/issues/262))

Bugfixes:

  - don't set up all groups every time Lic.setup is called while locked ([#263](https://github.com/lic/lic/issues/263))
  - fix #full_gem_path for git gems while locked ([#268](https://github.com/lic/lic/issues/268))
  - eval gemspecs at the top level, not inside the Lic class ([#269](https://github.com/lic/lic/issues/269))
...
Features:
  ...
  - console command that runs irb with lic (and optional group) already loaded

Bugfixes:

  - Lic.setup now fully disables system gems, even when unlocked ([#266](https://github.com/lic/lic/issues/266), [#246](https://github.com/lic/lic/issues/246))
    - fixes Yard, which found plugins in Gem.source_index that it could not load
    - makes behaviour of `Lic.require` consistent between locked and unlocked loads

## 0.9.17 (April 7, 2010)

Features:

  - Lic.require now calls Lic.setup automatically
  - Gem::Specification#add_lic_dependencies added for gemspecs

Bugfixes:

  - Gem paths are not longer duplicated while loading lic
...
Features:
  ...
  - print error instead of backtrace when exec cannot find a binary ([#241](https://github.com/lic/lic/issues/241))
...
Bugfixes:
  ...
  - report errors from lic/setup.rb without backtraces ([#243](https://github.com/lic/lic/issues/243))
  - fix Gem::Spec#git_version to not error on unloaded specs
  - improve deprecation, Gemfile, and command error messages ([#242](https://github.com/lic/lic/issues/242))
...
Features:
  ...
  - use the env_file if possible instead of doing a runtime resolve
     - huge speedup when calling Lic.setup while locked
     - ensures lic exec is fast while locked
     - regenerates env_file if it was generated by an older version
  - update cached/packed gems when you update gems via lic install

Bugfixes:

  - prep for Rubygems 1.3.7 changes
  - install command now pulls git branches correctly ([#211](https://github.com/lic/lic/issues/211))
...
Features:
  ...
  - install command output vastly improved
    ...
    - lic gems no longer listed as 'system gems'
  ...
  - Lic.with_clean_env for shelling out to ruby scripts
...
Bugfixes:
  ...
  - perform a topological sort on resolved gems ([#191](https://github.com/lic/lic/issues/191))
  - gems from git work even when paths or repos have spaces ([#196](https://github.com/lic/lic/issues/196))
  - Specification#loaded_from returns a String, like Gem::Specification ([#197](https://github.com/lic/lic/issues/197))
  ...
  - unify the Installer's local index and the runtime index ([#204](https://github.com/lic/lic/issues/204))
...
Features:
  ...
  - install command can take the path to the gemfile with --gemfile ([#125](https://github.com/lic/lic/issues/125))
  - unknown command line options are now rejected ([#163](https://github.com/lic/lic/issues/163))
  - exec command hugely sped up while locked ([#177](https://github.com/lic/lic/issues/177))
  - show command prints the install path if you pass it a gem name ([#148](https://github.com/lic/lic/issues/148))
  - open command edits an installed gem with $EDITOR ([#148](https://github.com/lic/lic/issues/148))
  - Gemfile allows assigning an array of groups to a gem ([#114](https://github.com/lic/lic/issues/114))
...
Bugfixes:
  ...
  - normalize parameters to Lic.require ([#153](https://github.com/lic/lic/issues/153))
  - check now checks installed gems rather than cached gems ([#162](https://github.com/lic/lic/issues/162))
  - don't update the gem index when installing after locking ([#169](https://github.com/lic/lic/issues/169))
  - lic parenthesises arguments for 1.8.6 ([#179](https://github.com/lic/lic/issues/179))
  - gems can now be assigned to multiple groups without problems ([#135](https://github.com/lic/lic/issues/135))
...
Bugfixes:
  ...
  - don't reinstall gems from the cache if they're already in the lic
...
Features:
  ...
  - allow manual configuration via LIC_PATH
...
Bugfixes:
  ...
  - fix `lic install` for a locked app without a .lic directory
...
Features:
  ...
  - added `lic install --disable-shared-gems`
  - Lic.require fails silently if a library does not have a file on the load path with its name
  - Basic support for multiple rubies by namespacing the default lic path using the version and engine

Bugfixes:

  - if the lic is locked and .lic/environment.rb is not present when Lic.setup is called, generate it
  - same if it's not present with `lic check`
  - same if it's not present with `lic install`
