...
RSpec.describe "gemcutter's dependency API" do
  ...
  it "should use the endpoint when using --deployment" do
    ...
    lic! :install, forgotten_command_line_options(:deployment => true, :path => "vendor/lic").merge(:artifice => "endpoint")
    expect(out).to include("Fetching gem metadata from #{source_uri}")
    expect(the_lic).to include_gems "rack 1.0.0"
  end
  ...
  it "handles git dependencies that are in rubygems using --deployment" do
    ...
    lic "install --deployment", :artifice => "endpoint"
  ...
  it "doesn't fail if you only have a git gem with no deps when using --deployment" do
    ...
    lic "install", :artifice => "endpoint"
    lic! :install, forgotten_command_line_options(:deployment => true).merge(:artifice => "endpoint")
  ...
  it "falls back when hitting the Gemcutter Dependency Limit" do
    gemfile <<-G
      ...
      gem "rails"
    G
    lic :install, :artifice => "endpoint_fallback"
    expect(out).to include("Fetching source index from #{source_uri}")
  ...
  it "falls back when Gemcutter API doesn't return proper Marshal format" do
    ...
    lic :install, :verbose => true, :artifice => "endpoint_marshal_fail"
  ...
  it "falls back when the API URL returns 403 Forbidden" do
    ...
    lic :install, :verbose => true, :artifice => "endpoint_api_forbidden"
    expect(out).to include("Fetching source index from #{source_uri}")
    expect(the_lic).to include_gems "rack 1.0.0"
  end
  ...
  it "handles host redirects" do
    ...
    lic :install, :artifice => "endpoint_host_redirect"
    expect(the_lic).to include_gems "rack 1.0.0"
  end
  ...
  it "handles host redirects without Net::HTTP::Persistent" do
    ...
    lic :install, :artifice => "endpoint_host_redirect", :requires => [lib_path("disable_net_http_persistent.rb")]
    expect(out).to_not match(/Too many redirects/)
    expect(the_lic).to include_gems "rack 1.0.0"
  end

  it "timeouts when Lic::Fetcher redirects too much" do
    gemfile <<-G
      source "#{source_uri}"
      gem "rack"
    G

    lic :install, :artifice => "endpoint_redirect"
  ...
  context "when --full-index is specified" do
    it "should use the modern index for install" do
      ...
      lic "install --full-index", :artifice => "endpoint"
      expect(out).to include("Fetching source index from #{source_uri}")
      expect(the_lic).to include_gems "rack 1.0.0"
    end
    ...
    it "should use the modern index for update" do
      ...
      lic! "update --full-index", :artifice => "endpoint", :all => lic_update_requires_all?
      expect(out).to include("Fetching source index from #{source_uri}")
      expect(the_lic).to include_gems "rack 1.0.0"
    end
  end
  ...
  it "fetches again when more dependencies are found in subsequent sources", :lic => "< 2" do
    ...
    lic :install, :artifice => "endpoint_extra"
  ...
  it "fetches gem versions even when those gems are already installed" do
    gemfile <<-G
      ...
      gem "rack", "1.0.0"
    G
    lic :install, :artifice => "endpoint_extra_api"
  ...
  it "does not fetch every spec if the index of gems is large when doing back deps", :lic => "< 2" do
    build_repo2 do
      build_gem "back_deps" do |s|
        s.add_dependency "foo"
      end
    ...
    lic :install, :artifice => "endpoint_extra_missing"
  ...
  it "uses the endpoint if all sources support it" do
    ...
    lic :install, :artifice => "endpoint_api_missing"
    expect(the_lic).to include_gems "foo 1.0"
  end
  ...
  it "fetches again when more dependencies are found in subsequent sources using --deployment", :lic => "< 2" do
    ...
    lic "install --deployment", :artifice => "endpoint_extra"
    expect(the_lic).to include_gems "back_deps 1.0"
  end
  ...
  it "installs the binstubs", :lic => "< 2" do
    ...
    lic "install --binstubs", :artifice => "endpoint"
  ...
  it "installs the bins when using --path and uses autoclean", :lic => "< 2" do
    ...
    lic "install --path vendor/lic", :artifice => "endpoint"
  ...
  it "installs the bins when using --path and uses lic clean", :lic => "< 2" do
    ...
    lic "install --path vendor/lic --no-clean", :artifice => "endpoint"
  ...
  context "when using basic authentication" do
    ...
    it "passes basic authentication details and strips out creds" do
      ...
      lic :install, :artifice => "endpoint_basic_authentication"
    ...
    it "does not pass the user / password to different hosts on redirect" do
      ...
      lic :install, :artifice => "endpoint_creds_diff_host"
      expect(the_lic).to include_gems "rack 1.0.0"
    end
    ...
    describe "with authentication details in lic config" do
      ...
      it "reads authentication details by host name from lic config" do
        ...
        lic :install, :artifice => "endpoint_strict_basic_authentication"
  ...
  context ".gemrc with sources is present" do
    ...
    it "uses other sources declared in the Gemfile" do
      ...
      lic "install", :artifice => "endpoint_marshal_fail"
