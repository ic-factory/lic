...
RSpec.describe "lic help" do
  # RubyGems 1.4+ no longer load gem plugins so this test is no longer needed
  it "complains if older versions of lic are installed", :rubygems => "< 1.4" do
    system_gems "lic-0.8.1"

    lic "help"
    expect(err).to include("older than 0.9")
    expect(err).to include("running `gem cleanup lic`.")
  end
  ...
  it "prefixes lic commands with lic- when finding the groff files" do
    with_fake_man do
      lic "help install"
    end
    expect(out).to eq(%(["#{root}/man/lic-install.1"]))
  end
  ...
  it "simply outputs the txt file when there is no man on the path" do
    ...
    expect(out).to match(/LIC-INSTALL/)
  end

  it "still outputs the old help for commands that do not have man pages yet" do
    lic "help version"
    expect(out).to include("Prints the lic's version information")
  end

  it "looks for a binary and executes it with --help option if it's named lic-<task>" do
  ...
  it "is called when only using the --help flag" do
    ...
    expect(out).to eq(%(["#{root}/man/lic.1"]))
