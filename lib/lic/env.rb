...
require "lic/source/git/git_proxy"
...
module Lic
  class Env
    ...
    def self.report(options = {})
      ...
      append_formatted_table("Lic Build Metadata", BuildMetadata.to_h, out)

      unless Lic.settings.all.empty?
        out << "\n## Lic settings\n\n```\n"
      ...
      return out unless SharedHelpers.in_lic?

      if print_gemfile
        gemfiles = [Lic.default_gemfile]
        begin
          gemfiles = Lic.definition.gemfiles
        ...
        out << "\n### #{Lic.default_lockfile.relative_path_from(SharedHelpers.pwd)}\n\n"
        out << "```\n" << read_file(Lic.default_lockfile).chomp << "\n```\n"
      end

      if print_gemspecs
        dsl = Dsl.new.tap {|d| d.eval_gemfile(Lic.default_gemfile) }
    ...
    def self.read_file(filename)
      Lic.read_file(filename.to_s).strip
    rescue Errno::ENOENT
      "<No #{filename} found>"
    ...
    def self.git_version
      Lic::Source::Git::GitProxy.new(nil, nil, nil).full_version
    rescue Lic::Source::Git::GitNotInstalledError
      "not installed"
    end

    def self.version_of(script)
      return "not installed" unless Lic.which(script)
      `#{script} --version`.chomp
    end

    def self.chruby_version
      return "not installed" unless Lic.which("chruby-exec")
    ...
    def self.environment
      ...
      %w[rubygems-lic open_gem].each do |name|
        specs = Lic.rubygems.find_name(name)
        out << ["  #{name}", "(#{specs.map(&:version).join(",")})"] unless specs.empty?
      end
      if (exe = caller.last.split(":").first) && exe =~ %r{(exe|bin)/lic?\z}
