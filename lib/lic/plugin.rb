...
require "lic/plugin/api"

module Lic
  module Plugin
    autoload :DSL,        "lic/plugin/dsl"
    autoload :Events,     "lic/plugin/events"
    autoload :Index,      "lic/plugin/index"
    autoload :Installer,  "lic/plugin/installer"
    autoload :SourceList, "lic/plugin/source_list"
  ...
  module_function
    ...
    def install(names, options)
      ...
      if specs
        ...
        specs_to_delete.values.each {|spec| Lic.rm_rf(spec.full_gem_path) }
      end

      Lic.ui.error "Failed to install plugin #{name}: #{e.message}\n  #{e.backtrace.join("\n ")}"
    end
    ...
    def gemfile_install(gemfile = nil, &inline)
      Lic.settings.temporary(:frozen => false, :deployment => false) do
        builder = DSL.new
        if block_given?
      ...
      unless e.is_a?(GemfileError)
        Lic.ui.error "Failed to install plugin: #{e.message}\n  #{e.backtrace[0]}"
      end
      raise
    end
    ...
    # Otherwise, points to global root, in Lic.user_lic_path("plugin")
    def root
      @root ||= if SharedHelpers.in_lic?
        local_root
      else
        global_root
      end
    ...
    def local_root
      Lic.app_config_path.join("plugin")
    end

    # The global directory root for all plugin related data
    def global_root
      Lic.user_lic_path("plugin")
    end
    ...
    def add_hook(event, &block)
      unless Events.defined_event?(event)
        raise ArgumentError, "Event '#{event}' not defined in Lic::Plugin::Events"
      end
    ...
    def hook(event, *args, &arg_blk)
      return unless Lic.feature_flag.plugins?
      unless Events.defined_event?(event)
        raise ArgumentError, "Event '#{event}' not defined in Lic::Plugin::Events"
      end
    ...
    def save_plugins(plugins, specs, optional_plugins = [])
      plugins.each do |name|
        ...
        Lic.ui.info "Installed plugin #{name}" if installed
      end
    end
    ...
    def load_plugin(name)
      ...
      Lic.ui.error "Failed loading plugin #{name}: #{e.message}"
      raise
    end
